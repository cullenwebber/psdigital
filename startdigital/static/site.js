(() => {
  ...
  function horizontalLoop(items, config3) {
    ...
    let tl = gsapWithCSS.timeline({
      ...
      defaults: { ease: "none" }
    }), length = items.length, startX = items[0].offsetLeft, times = [], widths = [], xPercents = [], curIndex = 0, pixelsPerSecond = (config3.speed || 1) * 100, snap3 = config3.snap === false ? (v) => v : gsapWithCSS.utils.snap(config3.snap || 1), totalWidth, curX, distanceToStart, distanceToLoop, item, i;
    gsapWithCSS.set(items, {
      xPercent: (i2, el) => {
        let w = widths[i2] = parseFloat(gsapWithCSS.getProperty(el, "width", "px"));
        xPercents[i2] = snap3(
          parseFloat(gsapWithCSS.getProperty(el, "x", "px")) / w * 100 + gsapWithCSS.getProperty(el, "xPercent")
        );
        return xPercents[i2];
      }
    });
    gsapWithCSS.set(items, { x: 0 });
    totalWidth = items[length - 1].offsetLeft + xPercents[length - 1] / 100 * widths[length - 1] - startX + items[length - 1].offsetWidth * gsapWithCSS.getProperty(items[length - 1], "scaleX");
    for (i = 0; i < length; i++) {
      item = items[i];
